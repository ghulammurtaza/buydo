<?php
class Gala_Galavineyardsettings_Helper_Sample extends Mage_Core_Helper_Abstract
{
	protected $_themeSlug  = 'default/galavineyard';
	...
	protected $_megaMenuModel = 'galavineyardsettings/megamenupro';
	protected $_megaSlideshowModel = 'galavineyardsettings/slider';
	protected $_tmpBlockMenu	=	array(
					"galavineyard_sample_vertical_menu_left",
					"galavineyard_sample_vertical_menu_right"
				);
	protected $_tmpBlockSlide	=	array(
					"galavineyard_main_slideshow"
				);
	...
	*/
	...
	public function insertStaticBlock($dataBlock) {
		...
		unset($dataBlock['id']);
		if (!$block->getId()){
			if(in_array($dataBlock['identifier'],$this->_tmpBlockMenu))	$dataBlock	=	$this->getFilterMenu($dataBlock);
			if(in_array($dataBlock['identifier'],$this->_tmpBlockSlide)) $dataBlock	=	$this->getFilterSlideshow($dataBlock);
	...
	public function getFilterMenu($dataBlock){
		$match	=	preg_match('/menu=["\']?[0-9]+["\']?/', $dataBlock['content'],$r);
		$tmp_id =	preg_replace('/menu=["\']?|["\']?/', '', $r[0]);
		$dataBlock['content']	=	str_replace($r[0],"menu=\"".$this->_menuIds[$tmp_id]."\"",$dataBlock['content']);
		return $dataBlock;
	}

	public function getFilterSlideshow($dataBlock){
		$match	=	preg_match('/slideshow=["\']?[0-9]+["\']?/', $dataBlock['content'],$r);
		$tmp_id =	preg_replace('/slideshow=["\']?|["\']?/', '', $r[0]);
		$dataBlock['content']	=	str_replace($r[0],"slideshow=\"".$this->_slideshowIds[$tmp_id]."\"",$dataBlock['content']);
		return $dataBlock;
	}
