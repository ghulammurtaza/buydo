...
class Gala_Galavineyardsettings_Block_Popular extends Mage_Core_Block_Template
{
    protected $_tags;
        ...
        if (count($this->getTags()) > 0) {
            if($this->isEnableFlashtag()){
                if($setting->checkMobile()=='false'){
                    $this->setTemplate('tag/galavineyard/popular.phtml');
                }
            }
    ...
    public function getFlashTag(){
        ...
        $maxQty = max(array_values($this->getTagSize()));
        $minQty = min(array_values($this->getTagSize()));
        $path = Mage::getBaseUrl(Mage_Core_Model_Store::URL_TYPE_MEDIA).'galavineyard/tagcloud.swf';
        ...
        $html = '<embed id="flashtags_popular" width="260" height="260" flashvars="'.$flashVars.'" wmode="transparent" allowscriptaccess="always"
    ...
    public function isEnableFlashtag(){
        return Mage::getStoreConfig('galavineyard/flashtag/enabled');
    }

    public function getWidth(){
        return Mage::getStoreConfig('galavineyard/flashtag/width');
    }

    public function getHeight(){
        return Mage::getStoreConfig('galavineyard/flashtag/height');
    }

    public function getCount(){
        return Mage::getStoreConfig('galavineyard/flashtag/count');
    }

    public function getBackground(){
        return Mage::getStoreConfig('galavineyard/flashtag/back');
    }

    public function getMaxSize(){
        return Mage::getStoreConfig('galavineyard/flashtag/max');
    }

    public function getMinSize(){
        return Mage::getStoreConfig('galavineyard/flashtag/min');
    }

    public function getMaxColor(){
        return Mage::getStoreConfig('galavineyard/flashtag/maxcolor');
    }

    public function getNormalColor(){
        return Mage::getStoreConfig('galavineyard/flashtag/normal');
    }

    public function getHoverColor(){
        return Mage::getStoreConfig('galavineyard/flashtag/hover');
    }
    
    public function getSpeed(){
        return Mage::getStoreConfig('galavineyard/flashtag/speed');
    }
}
