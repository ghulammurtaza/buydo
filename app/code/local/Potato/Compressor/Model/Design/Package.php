...
class Potato_Compressor_Model_Design_Package extends Mage_Core_Model_Design_Package
{
    ...
    {
        //skip files with extension min.css
        $skippedFilesContent = '';
        foreach ($srcFiles as $key => $file) {
            if (strpos($file, '.min.css')) {
                $skippedFilesContent .= file_get_contents($file) . "\n";
                unset($srcFiles[$key]);
            }
        }
        ...
        if ($extensionsFilter == Potato_Compressor_Model_Compressor_Css::FILE_EXTENSION &&
            ...
            //include to result content from skipped files
            $result .= Mage::getSingleton('po_compressor/compressor_css')->removeComments($skippedFilesContent);
